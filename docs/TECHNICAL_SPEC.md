...
当前参考文件首表为 `2607工资`，约 158 条人员记录。字段位置为：
...
解析不能只依赖固定列号，还应以表头名称查找；固定列号作为兼容回退。导入规则：

1. 只接受 `.xlsx`，最大 10 MB。
2. 只读取第一个工作表。
3. 期间必须能够标准化为 `YYYY-MM`，一个文件只允许一个期间。
4. 姓名不能为空，同一文件内不得重复。
5. 成本必须是有限数值且不小于 0；公式错误、空白和非数值必须报错，不能转换为 0。
6. 匹配优先级：已有人名映射 > 唯一姓名 > 新建演示人员。
7. 正式钉钉模式下，同名或未匹配人员必须人工确认，不能自动新建。
8. 预览返回有效行、错误行、月份和文件摘要；存在错误时不允许提交。
9. 提交采用批次原子写入，同一人员同一月份执行带审计的更新。
