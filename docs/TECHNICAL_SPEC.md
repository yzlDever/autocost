...
- 只接受由钉钉 OAuth 回调创建、且包含钉钉 `userId` 的会话；旧固定账号会话一律拒绝。
...
- 会话和审计不保存钉钉 AccessToken、Client Secret 或 OAuth 授权码。
...
- 钉钉 `userid` 直接作为人员主键；重复同步时按 `userid` 更新姓名、工号、部门和状态。
- 本次结果中缺失的既有钉钉人员标记为 `inactive`，不物理删除人员或工资历史。
- 钉钉人员主键升级时，`schemaVersion: 1` 的测试数据会一次性重置为空的 `schemaVersion: 2`，再重新同步通讯录、导入工资并创建接口密钥。
- 新数据仓库为空；进入 `schemaVersion: 2` 后，历史遗留的演示人员、演示成本和演示审计仍会自动清理，不影响同版本中的真实人员、离职人员或工资历史。
...
- `id`：钉钉 `userid`，作为人员主键和第三方接口的 `employeeId`。
- `employeeNo`：工号，可为空。
- `name`：姓名。
- `department`：当前部门。
- `status`：`active` / `inactive`。
- `source`：`dingtalk` / `excel`。
- `lastSyncedAt`。
...
人员管理页面提供系统工资模板下载。模板从数据库读取全部在册及历史人员，写入钉钉人员编码（`userid`）、工号、姓名、部门和在离职状态；工资期间和公司人力总成本由财务填写。模板第一工作表结构为：

- `B1`：工资期间，例如 `202607`。
- `A:F`：钉钉人员编码、工号、姓名、部门、人员状态、公司人力总成本。
...
4. 系统模板按钉钉人员编码精确匹配；旧版工资表仅兼容唯一工号、唯一姓名匹配。
