...
1. 在人员管理页同步钉钉通讯录。
2. 点击“下载工资模板”，系统会写入全部在册及历史人员的钉钉人员编码（`userid`）、工号、姓名、部门和状态。
3. 在模板 `B1` 填写 `YYYYMM` 工资期间，并填写“公司人力总成本”。
4. 在工资管理页上传模板，系统按钉钉人员编码精确关联月度快照。

钉钉 `userid` 直接作为人员主键和第三方接口的 `employeeId`。人员离开钉钉可见范围后只会标记为离职，不会删除人员或历史工资。旧版工资表仍可按唯一工号或唯一姓名兼容匹配，但不能自动创建未同步人员。

## 聚合查询接口

`POST /api/v2/labor-cost/query` 按“人员 ID + 月份/天数列表”查询。人员 ID 使用钉钉 userId，每个月的 `days` 必须是 1 至 23 的整数；系统按照“月度公司人力总成本 ÷ 23 × days”计算，最终只返回至少两人的整批总成本。`/api/v1/labor-cost/query` 暂作为相同新格式的兼容地址。
  ...
  "items": [
    {
      ...
      "periods": [{ "period": "2026-07", "days": 15 }]
    },
    {
...
接口继续执行 API 密钥认证、至少两名不同人员、防差分推算和查询审计；不再设置单人 10% 最低贡献限制。
...
新数据仓库从空人员、空工资状态开始。此次钉钉人员主键升级会在首次读取时将 `schemaVersion: 1` 的测试数据整体重置为空的 `schemaVersion: 2`；需要重新同步钉钉通讯录、导入工资并创建接口密钥。后续同版本运行仅自动清理历史遗留的演示人员及对应演示成本。
